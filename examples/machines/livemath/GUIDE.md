...
_skill `livemath`; format `efsm-v1`; machine version `0.1.0`; structure fingerprint `68fa9d9343`; generated by hexis 0.1.0_
...
flowchart TD
    ...
    n9 -->|"3. verify_verdict == '弃权' and repair_count #lt; 2…"| n8
    n9 -->|"4. otherwise"| n5
    n10 -->|"1. otherwise"| n11
```
...
| `s2m` | decide | decides `complete`, `incomplete`, `弃权`: answer_letter names the option chosen from request. If that option is the META option ('one of the remaining options is… | `request`, `analysis`, `answer_letter` | `verify_note` | S2 |
...
| `s6` | decide | decides `pass`, `wrong_content`, `弃权`: file_content is the answer file read back after writing. Answer 'pass' if it contains exactly one \\boxed{X} whose lett… | `file_content`, `answer_letter` | `verify_verdict` | S3 |
...
- `s6`: 1. if `verify_verdict == 'pass'` → `s7`; 2. if `verify_verdict == 'wrong_content' and repair_count < 2` → `s3`, add 1 to `repair_count`; 3. if `verify_verdict == '弃权' and repair_count < 2` → `s5`, add 1 to `repair_count`; 4. otherwise → `s8`
