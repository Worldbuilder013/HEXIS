...
_skill `structure-preserving-spreadsheet-edits`; format `efsm-v1`; machine version `0.1.0`; structure fingerprint `72a90d46dd`; generated by hexis 0.1.0_
...
flowchart TD
    ...
    n12 -->|"5. audit_verdict == '弃权' and s7g_count #lt; 3 [+1…"| n9
    n12 -->|"6. otherwise"| n14
    n13 -->|"1. otherwise"| n15
    n14 -->|"1. otherwise"| n16
```
...
| `s4` | decide | decides `sufficient`, `insufficient`, `弃权`: Decide whether inspect_report gives enough structural evidence to commit to boundary_model. Answer 'sufficient' by defa… | `inspect_report`, `boundary_model`, `followup_targets` | `boundary_verdict` | S2.1.3 |
...
| `s6_loop` | decide | decides `again`, `continue`, `弃权`: Step s6: bash/apply {"command": "${edit_cmd}"} may need to run more than once. Given its output, answer 'again' if the… | `stdout`, `edit_log` | `s6_loop_choice` |  |
| `s6_j` | decide | decides `s7g`, `s10`, `弃权`: After s6: bash/apply {"command": "${edit_cmd}"}, decide which step comes next. Options: 's7g' = s7g: model → ['verify_c… | `stdout`, `edit_log` | `s6_j_choice` |  |
...
| `s7_loop` | decide | decides `again`, `continue`, `弃权`: The audit in s7 is a program written for this workbook: answer 'again' only if its report is empty, unreadable or missi… | `stdout`, `verify_report` | `s7_loop_choice` |  |
| `s8` | decide | decides `pass`, `wrong_cells`, `boundary_wrong`, `弃权`: Judge verify_report. Step 1 - find every line that starts with 'FLAG' in the report. If there is at least one FLAG line… | `verify_report`, `edit_plan`, `boundary_model` | `audit_verdict` | S2.3.3 |
...
- `s4`: 1. if `(boundary_verdict == 'insufficient' or boundary_verdict == '弃权') and inspect_count < 2` → `s1`, add 1 to `inspect_count`; 2. if `boundary_verdict == 'insufficient' and inspect_count >= 2` → `s5`; 3. if `boundary_verdict == 'sufficient'` → `s5`; 4. otherwise → `FALLBACK`
...
- `s8`: 1. if `audit_verdict == 'pass'` → `s9`; 2. if `audit_verdict == 'wrong_cells' and repair_count < 2` → `s5`, add 1 to `repair_count`; 3. if `((audit_verdict == 'wrong_cells' and repair_count >= 2) or audit_verdict == 'boundary_wrong') and boundary_count < 1` → `s3`, add 1 to `boundary_count`; 4. if `((audit_verdict == 'wrong_cells' and repair_count >= 2) or audit_verdict == 'boundary_wrong') and boundary_count >= 1 and inspect_count < 2` → `s1`, add 1 to `inspect_count`; 5. if `audit_verdict == '弃权' and s7g_count < 3` → `s7g`, add 1 to `s7g_count`; 6. otherwise → `s10`
