...
``results.jsonl`` are skipped.
    ...
    hexis-agent bench --mode livemath --tasks-file livemath.yaml --tasks "$(cat test_tasks.txt)" \\
...
def run_skill_job(a, task: dict, tid: str, rep: int, job_dir: pathlib.Path, env: dict, grader,
    ...
    if livemath:
        ...
        if not (job_dir / "answer.txt").is_file():
            ...
            from hexis.evaluators.mcq_boxed import extract_choice
...
def run_fsm_job(a, task: dict, tid: str, rep: int, job_dir: pathlib.Path) -> dict:
    ...
    cmd = [PY, "-m", "hexis", "run", "--machine", a.machine, "--skill", a.skill,
...
def main(argv: list[str] | None = None, prog: str | None = None) -> int:
    ...
    ap.add_argument("--rbank", default=None,
                    help="rbank arm: precomputed retrieval JSON, task id -> injected text (hexis-agent memory precompute)")
    ...
    if a.mode == "livemath":
        from hexis.evaluators.mcq_boxed import grade_answer_file as grader
    elif a.mode == "filetask":
        grader = None
    else:
        from hexis.evaluators.spreadsheet_golden import compare_workbooks as grader
