...
For every task the output directory keeps the raw evidence next to a folded trace::
    ...
    <out>/<task_id>.jsonl          folded trace, the input of ``hexis-agent compile --traces``
...
unless ``--no-resume`` is given, and a change of :data:`PROTOCOL` invalidates earlier reports.
    ...
    hexis-agent collect --tasks-file tasks.yaml --skill SKILL_DIR --out traces/ --model MODEL
"""
...
def agent_config(opencode_model: str, upstream_model: str, skill_text: str, head: str = PROMPT_HEAD) -> str:
    ...
    return json.dumps({
        ...
        "agent": {
            AGENT: {
                "description": "hexis trace collection: skill + two primitives",
                "mode": "primary",
...
def fold_events(events_text: str, *, task_id: str, request: str, input_path: str,
    ...
    produces the action sequence that :func:`hexis.traces.trace_adapter.read_raw_jsonl` reads.
...
def main(argv: list[str] | None = None, prog: str | None = None) -> int:
    ...
    ap.add_argument("--provider", default="default",
                    help="endpoint profile (hexis.llm.env); the model name must belong to that endpoint")
