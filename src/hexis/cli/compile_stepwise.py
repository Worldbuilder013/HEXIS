...
``<out>/machine.json``, so work can resume at any time.
    ...
    hexis-agent compile-stepwise --skill SKILL_DIR --traces TRACE_DIR --out OUT --machine-init M0.json --show 8
    ...
    hexis-agent compile-stepwise ... --apply decisions.json

    # 3. write the report once every trace is processed
    hexis-agent compile-stepwise ... --report
"""
