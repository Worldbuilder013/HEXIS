"""Build AWM workflows and ReasoningBank memories from trajectories.
    ...
    hexis-agent memory convert RUN_DIR TRAJ.jsonl       # skill-arm event streams of a bench run -> trajectories
    hexis-agent memory judge TRAJ.jsonl                 # model self-judgement of success, written back as self_judge
    hexis-agent memory awm TRAJ.jsonl WORKFLOWS.txt     # AWM offline induction from successful trajectories
    hexis-agent memory rbank TRAJ.jsonl BANK.jsonl      # ReasoningBank: strategies from successes, lessons from failures
    hexis-agent memory index BANK.jsonl                 # embedding index, written as BANK.npy
    hexis-agent memory precompute BANK.jsonl TASKS.yaml TASK_IDS OUT.json [--mode xlsx] [--k 5]
    hexis-agent memory retrieve BANK.jsonl "query" [--k 5]
...
def main(argv: list[str] | None = None, prog: str | None = None) -> int:
    ...
    p.add_argument("run_dir", help="output directory of hexis-agent bench")
    ...
    if a.cmd == "convert":
        ...
        if a.mode == "xlsx":
            from hexis.cli.collect import task_prompt
