...
驱动机器走一遍（:mod:`hexis.legacy.replay`，定理2 保证给定结果序列时状态与变量序列是它的
函数），再对机器自己的账面数字设门槛。
...
:func:`hexis.legacy.replay.replay` 走到 FALLBACK 就无条件接受剩余），于是它**不可能**偏离，
...
class VerifyReport:
    ...
    ok: bool
    #: 复述得出、但真跑不等同的接受轨迹下标（见 hexis.legacy.conformance）。空 = 全部等同。
...
def verify_machine(m: Machine, t_plus: Sequence[Trace] = (),
    ...
    # 相同。见 :mod:`hexis.legacy.conformance`。
