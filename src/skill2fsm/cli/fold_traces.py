"""Fold the OpenCode event streams of a benchmark run into traces.
    ...
    skill2fsm fold-traces --bench RUN_DIR --tasks-file TASKS.yaml --out TRACE_DIR
...
from skill2fsm.cli.collect import fold_events


def main(argv: list[str] | None = None, prog: str | None = None) -> int:
    ap = argparse.ArgumentParser(prog=prog, description=__doc__.split("\n")[0])
    ap.add_argument("--bench", required=True, help="benchmark run directory (output of skill2fsm bench with a skill arm)")
