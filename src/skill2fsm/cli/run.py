...
configured through ``.env`` (see :mod:`skill2fsm.env`).

    skill2fsm run --mode livemath --machine machine.json --skill skills/livemath \\
...
def run_machine(a) -> int:
    ...
    from skill2fsm.llm_client import ModelAdapter
    from skill2fsm.opencode_tools import FALLBACK_PROTOCOL, PRIMITIVES, OpenCodeTools
    ...
    with client_from_env(profile=prov, timeout=float(getattr(a, "llm_timeout", 900.0)),
        ...
        if getattr(a, "executor", "opencode") == "local":
            from skill2fsm.local_tools import LocalTools
        ...
        if mode == "livemath":
            if getattr(a, "answer", None) is not None:
                from skill2fsm.evaluators.mcq_boxed import grade_answer_file
