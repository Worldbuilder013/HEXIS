...
:mod:`skill2fsm.compiler` 是「沿轨迹走、建状态、接边」的流程骨架，它跟 table_clean 那套
...
3. **互斥可证**：各支的条件在**符号层**两两不同时成立（把变量压成有限格局逐个枚举，与
   :func:`skill2fsm.checks.structural_findings` 判互斥同一套办法），不是只在观测到的那几个
   快照上碰巧不重叠。定理2 要的是前者。
...
from . import cond
from .checks import _back_edges, _reachable
from .schema import Machine, Transition, Variable
