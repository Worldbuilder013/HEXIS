"""OpenAI 兼容 `/chat/completions` 客户端，以及把它接进 :class:`~skill2fsm.model_iface.Model`
协议的适配器。
...
def extract_json(text: str) -> Optional[dict]:
    ...
    扫描器复用 :func:`skill2fsm.runtime._first_json_object`。
    """
...
class OpenAIClient:
    ...
    def __init__(self, model: str, base_url: str, api_key: str, *,
        ...
        if not api_key:
            raise LLMError("没有 API key（见 skill2fsm.env.llm_config）")
        self.model = model
...
class ModelAdapter:
    """把 :class:`OpenAIClient` 接成 :class:`~skill2fsm.model_iface.Model`。
...
def client_from_env(*, timeout: float = 180.0, max_retries: int = 6,
    ...
    """按仓库根 ``.env`` 里的三件套建客户端。缺配置时 :class:`~skill2fsm.env.EnvError`。

    ``profile`` 选端点档案（``"minimax"`` ⇒ ``MINIMAX_*`` 三件套），见 :func:`skill2fsm.env.llm_config`。
    """
    from .env import llm_config
