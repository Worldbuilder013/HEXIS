...
* :func:`skill2fsm.compiler._sig`（严）——工具只按名字并，但判断动作要比 ``prompt``、终止
  动作要比 ``terminal``。编译期必须这么严：两个提问不同的判断是两个不同的语义分岔，并掉就
  等于把「表头规不规范」和「金额对不对」当成同一步。
* :func:`skill2fsm.replay._action_matches`（松）——只比 ``kind``，工具再加名字。回放期必须
...
``canon_action`` 同时吃 :class:`~skill2fsm.schema.Record`（``.action`` 是**裸 dict**）和
...
import** :mod:`skill2fsm.schema`：本模块因此零依赖、无环，任何一侧都能自由 import。
...
def canon_action(rec_or_action: Any, *, strict: bool = False) -> tuple[str, ...]:
    ...
    if kind == "tool":
        ...
        # 名字一样、用途不同，不细化就折成一个状态。阶段由 skill2fsm.phases 的纯函数在采集时
...
def context_key(rec_or_action: Any, preds: Sequence[Any] = (), *,
    ...
    回放**不看**这个 KEY：:func:`skill2fsm.replay._action_matches` 只比动作（松档），克隆
