...
**这一层为什么必须有。** :func:`skill2fsm.normalize.canon_action` 故意不把工具参数算进状态
...
:attr:`skill2fsm.schema.Machine.phase_rules` 记着用的是哪一套。

**判据是纯函数，不是模型。** 谁都能拿 ``out/artifacts/<sha>.py`` 里的正文重跑一遍核对。

**在采集时判，不在编译时判。** :func:`skill2fsm.trace_adapter._extract_code` 会把代码正文抽到
artifacts、记录里只留 ``code_sha256``——编译时已经读不到正文了。所以 ``to_trace`` 在抽走正文
**之前**算出阶段，写进 ``action["phase"]`` 随记录走。机器状态那侧同样带 ``phase``
（:class:`~skill2fsm.schema.ToolAction` 的字段），于是 ``canon_action`` 两侧对称地把它算进
