...
* **开局工具** :data:`~skill2fsm.normalize.BEGIN_TOOL`：机器的起点是它、而轨迹第一条记录
...
from .normalize import BEGIN_TOOL, canon_action, is_begin
from .runtime import pick_edge
from .schema import Machine, Record, Trace
...
def walk(machine: Machine, trace: Trace, *,
    ...
    if labelers is None:
        from .trace_adapter import LABELERS as labelers   # 延迟：trace_adapter 依赖本模块之外的一圈
...
def excludes(machine: Machine, neg: Trace, *,
    ...
    ``evaluate`` 是**注入的评判**，签名同 :func:`skill2fsm.judge.evaluate`
    ``(trace, acceptance, prohibitions) -> Verdict``；不给就用它。做成注入是为了让客观验收
    换得掉：MATH 那条线的验收是 :func:`skill2fsm.grader.acceptance_for`（答案与参考等价），
    直接组进来即可——

    .. code-block:: python

        from skill2fsm import grader, judge
    ...
    if machine.prohibitions:
        if evaluate is None:
            from .judge import evaluate as evaluate       # 默认实现：judge.evaluate
...
def _action_matches(m_act, r_act: dict) -> bool:
    ...
    :func:`skill2fsm.normalize.canon_action`（``strict=False``）。
