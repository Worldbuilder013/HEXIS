...
:func:`render_experiment` 与 :func:`render_html` 渲染 :mod:`skill2fsm.experiment` 跑出来的
...
from .schema import FALLBACK, Machine
...
def cover_report(machine: Machine, clauses: list, *, t_plus=(), t_minus=(),
    ...
    :class:`~skill2fsm.runtime.RunResult`，用来算 FALLBACK 落入率。
...
def _as_dict(report: Any) -> dict:
    """吃 :class:`~skill2fsm.experiment.ExperimentReport` 或它的 dict，都归一成 dict。"""
...
def render_html(report: Any, coverage: Optional[Mapping] = None) -> str:
    ...
    h.append("<title>三臂实验报告 · skill2fsm</title>")
    ...
    if cov:
        ...
        h.append('<p class="note">这一版报告里没有带覆盖账（跑 <code>skill2fsm report'
