...
**轨迹是「关于它自己」的。** 每条 :class:`~skill2fsm.schema.Record` 描述这台机器怎么走、
...
from .schema import FALLBACK, Machine, Record, Trace
...
class RunResult:
    ...
    * **测不到就说测不到。** ``prompt_tokens``/``completion_tokens`` 为 ``None`` 表示模型接口
      这一趟根本没报用量（:class:`~skill2fsm.model_iface.ScriptedModel` 没有 ``usage()``，真实
...
def _usage_of(model: Any) -> Optional[dict]:
    ...
    :class:`~skill2fsm.model_iface.ScriptedModel` 没有——不为桩补接口、也不给桩编数，缺就是
...
def _argv_of(tools: Any, name: str, out: Any) -> Optional[list]:
    ...
    :class:`~skill2fsm.sandbox.ExecResult` 记的是 ``command``，别的工具可能叫 ``argv``/``cmd``。
...
def _phase_of_call(act, inp: dict, phase_rules: str, outputs: tuple) -> dict:
    ...
    判据与外部日志那条路同源（:func:`skill2fsm.phases.classify`），所以两种来源的轨迹可比。
