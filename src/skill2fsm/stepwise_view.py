...
放在 ``skill2fsm/fsm/`` 之外：这里为了让人读得快，会识别命令里的工作簿读写、重算、目录列举等特征，
带有工具名与领域词；编译器核心（``skill2fsm/fsm/``）不得含这些词（test_46 钉住这一点）。
"""
...
from .fsm.align import NEW
from .fsm.stepwise import candidates, fingerprint, propose
from .fsm.traces import Event, Prepared, end_state_for
from .fsm.context import CompileContext
